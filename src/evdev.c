...
EvdevHookPointerConstrainCursor (DeviceIntPtr pDev, ScreenPtr pScreen, BoxPtr pBox)
{
    ...
    if(pEvdev->confined_id)
    {
	    xf86DrvMsg(-1, X_INFO, "[X11][EvdevHookPointerConstrainCursor][before] pBox (%d, %d, %d, %d), pPointer->limits (%d, %d, %d, %d), confined=%d\n",
	    ...
	    {
		  xf86DrvMsg(-1, X_INFO, "[X11][EvdevHookPointerConstrainCursor] confine routine --begin (pDev->id=%d)\n", pDev->id);
		  ...
		  xf86DrvMsg(-1, X_INFO, "[X11][EvdevHookPointerConstrainCursor] confine routine --end (pDev->id=%d)\n", pDev->id);
	    }
    }
    ...
    if(pEvdev->confined_id)
    {
	    xf86DrvMsg(-1, X_INFO, "[X11][EvdevHookPointerConstrainCursor][after] pBox (%d, %d, %d, %d), pPointer->limits (%d, %d, %d, %d), confined=%d\n",
			pBox->x1, pBox->y1, pBox->x2, pBox->y2, pPointer->limits.x1, pPointer->limits.y1, pPointer->limits.x2, pPointer->limits.y2, pPointer->confined);
    }
}
...
EvdevSetCursorLimits(InputInfoPtr pInfo, int region[6], int isSet)
{
	...
	if( isSet )
	{
		...
		if(!region[4])
		{
			xf86DrvMsg(-1, X_INFO, "[X11][SetCursorLimits] region[4]=%d NOT pointer warp !\n", region[4]);
			return;
		}
...
EvdevSetConfineRegion(InputInfoPtr pInfo, int num_item, int region[6])
{
	...
	if( num_item == 6 )
	{
		if ( (region[2]-region[0]>0) && (region[3]-region[1]>0) )
			...
			xf86DrvMsg(-1, X_INFO, "[X11][SetConfineRegion] Confine region was set to TOPLEFT(%d, %d) BOTTOMRIGHT(%d, %d) pointerwarp=%d, confine=%d\n",
